...
export default function InteractiveMap({activeLake,wineries,onOpen,children}){
 ...
 const pointerDown=event=>{if(event.button!==0||event.target.closest('button'))return;pointers.current.set(event.pointerId,{x:event.clientX,y:event.clientY});dragged.current=false;event.currentTarget.setPointerCapture(event.pointerId);if(pointers.current.size===1){const current=view.current;drag.current={id:event.pointerId,x:event.clientX,y:event.clientY,startX:current.position.x,startY:current.position.y}}else{drag.current=null;startPinch()}};
 ...
 return <div className={`interactiveMap${expanded?' expanded':''}`}>
  ...
  <div className="interactiveMapViewport" ref={viewport} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onDoubleClick={event=>changeZoom(view.current.zoom+1,event.clientX,event.clientY)} onWheel={event=>{event.preventDefault();changeZoom(view.current.zoom+(event.deltaY<0?.35:-.35),event.clientX,event.clientY)}}>
   <div className="interactiveMapLayer" style={{transform:`translate(${position.x}px,${position.y}px) scale(${zoom})`,'--pin-scale':1/zoom}}>
    {children}
    {wineries.map(winery=><button type="button" key={winery.id} title={`${winery.name} · ${winery.city}`} aria-label={`Open ${winery.name}`} className="pin directoryPin interactivePin" style={{left:winery.x+'%',top:winery.y+'%',background:winery.color}} onClick={event=>openWinery(event,winery)}><Wine size={12}/><span>{winery.name}</span></button>)}
   </div>
   ...
   <div className="mapHint"><Navigation size={15}/> {wineries.length} wineries · pinch or use + to zoom</div>
